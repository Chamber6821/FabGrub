...
class Requirement {
  public:
    ...
    // It is an ugly requirement from pubgrub lib
    ...
    [[nodiscard]] auto intersection(const Requirement &other) const
    ...
    [[nodiscard]] auto union_(const Requirement &other) const
        -> std::optional<Requirement> {
    ...
    [[nodiscard]] auto difference(const Requirement &other) const
        -> std::optional<Requirement> {
    ...
    [[nodiscard]] auto implied_by(const Requirement &other) const {
        return range.contains(other.range);
    }

    [[nodiscard]] auto excludes(const Requirement &other) const {
        return range.disjoint(other.range);
    }

    // Something copy-paste from pubgrub::test::simple_req
    // Concept doesn't require this,
    // but otherwise we will have a compilation error
